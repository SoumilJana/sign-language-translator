...
cooldown = 0.5    
sentence = ""
prediction_history = deque(maxlen=15)  # smoothing buffer
stable_letter = None
stable_count = 0
prev_frame_time = time.time()

# === New hold logic ===
HOLD_TIME = 3  # seconds to hold steady before registering
...
while True:
    ...
    if landmarks_raw is not None:
        ...
        # === Stable letter detection with hold time ===
        if smoothed_prediction == stable_letter:
            if hold_start_time is None:
                ...
                hold_start_time = None  # reset after registering
        else:
            stable_letter = smoothed_prediction
            hold_start_time = time.time()
