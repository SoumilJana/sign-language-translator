import os
# CRITICAL: Disable tokenizer parallelism to prevent deadlock with Kivy event loop
...
# T5 Sentence Generator for converting glosses to natural sentences
from sentence_generator import get_sentence_generator  
...
class CameraScreen(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            model_data = pickle.load(open("assets/model.p", "rb"))
            self.model = model_data["model"]
            self.label_classes = np.load("assets/label_classes.npy", allow_pickle=True)
        ...
        # --- T5 Sentence Generator (lazy loaded) ---
        self.sentence_generator = None
        self.use_ai_generation = True  # Toggle for AI sentence generation
        print("🤖 AI Sentence Generator will load on first use.")
    ...
    def trigger_speak(self):
        ...
        self._set_sentence_label("AI is processing...")
        
        # Run ALL heavy processing (T5 + TTS) in background thread
        threading.Thread(
            target=self._process_and_speak_thread, 
            args=(sentence_to_speak,), 
            daemon=True
        ).start()


    def _process_and_speak_thread(self, sentence_to_speak):
        """
        Runs T5 AI generation and TTS in a background thread.
        This prevents blocking the Kivy main loop.
        """
        try:
            # --- T5 AI Sentence Generation ---
            if self.use_ai_generation:
                try:
                    # Lazy load the T5 model on first use
                    if self.sentence_generator is None:
                        print("🔄 Loading AI model (first time, please wait)...")
                        self._set_sentence_label("Loading AI model...")
                        self.sentence_generator = get_sentence_generator()
                    
                    if self.sentence_generator.is_ready:
                        print(f"🔄 Processing with AI: '{sentence_to_speak}'")
                        self._set_sentence_label("AI is thinking...")
                        sentence_to_speak = self.sentence_generator.generate_sentence(sentence_to_speak)
                except Exception as e:
                    print(f"⚠️ AI generation failed, using original: {e}")
            
            # --- Text-to-Speech ---
            print(f"🟢 Speaking: '{sentence_to_speak}'")
            self._set_sentence_label(f"Speaking: {sentence_to_speak}")
            ...
            self._set_sentence_label("Error occurred")
